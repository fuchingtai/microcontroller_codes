...
int pinDHT11 = 13; //D13 Pin: Signal input from DHT11
SimpleDHT11 dht11;

String rawInput1; //String for entered values
int input1; //int variable the entered value (string --> int)

int temp_upper; //upper temp tolerance
int temp_lower; //lower temp tolerance

byte amb_temp = 0; //ambient temp from DHT11
byte humidity = 0; //humidity from DHT11
...
void loop() {
  ...
  if (Serial.available()) { //If Bluetooth connected
    rawInput1 = Serial.parseInt(); //String rawInput1 = *characters entered on the phone*
    input1 = rawInput1.toInt(); //String --> int

    delay(10000); //About 10 seconds to enter temp values upon BT connection
  }

  temp_upper = (input1) / 100; //Input1 integer would be ex: 3023 --> 3023/100 gives the integer 30. --> upper temp bound
  temp_lower = (input1) % 100; //Input1 integer would be ex: 3023 --> 3023%100 gives the integer 23. --> lower temp bound

  if (temp_upper != 0 && temp_lower != 0) { //execute the program if inputted values are accepted by Arduino

    if (dht11.read(pinDHT11, &amb_temp, &humidity, NULL)) { //return to previous line if DHT11 inputs are not recieved
      return;
    }
    ...
    byte decimal_temp = 0; //dummy value for the decimal nummber part of skin temp. (i.e.: 26.82) --> whole_temp = 82 **this will be displayed on the mobile app**
    ...
    //Heat Loss Equation
    double raw_heat_loss = (emissivity)*(blotz)*((pow((1.00 * raw_temp + 273.15), 4)) - (pow((1.00 * amb_temp + 273.15), 4))); //**ACTUAL EQUATION REQUIRED. NEED DOUBLE CHECK*
    ...
    Serial.print((int)amb_temp);             /////////////Ambient Temp///////////// One index **DHT11 only has whole numbers**
    Serial.print(" C");
    Serial.print("|"); //#Index 1           **The '|' will be read by the app as an index**  //ex output: "26 C"

    Serial.print((int)whole_temp); //prints "26.57" /////////////Skin Temp///////////// Two indexes
    Serial.print("."); //"26." //#Index 2
    Serial.print("|");
    ...
    Serial.print(" C");
    Serial.print("|"); //"57 C" //#Index 3 //ex output: "26.98 C"

    Serial.print("0.");                           /////////////WeaVE Emissivity///////////// One index
    if ((int)decimal_emissivity < 10) { //0.0X
      Serial.print("0");
      Serial.print((int)decimal_emissivity); //#Index 4 //ex output: "0.08"
    }
    else {
    ...
    if (amb_temp*1.00 > raw_temp) { /////////////Heat Loss///////////// Two Index
      whole_heat_loss = 0;
      decimal_heat_loss = 0;
    ...
    Serial.print(" W/m^2");
    Serial.print("|"); //"57 C" //#Index 6

    if (voltage <= 0) { //NEGATIVE voltage       /////////////Voltage///////////// Two indexes (since I assigned whole_voltage already, but this would've been one index)
      Serial.print("-"); //neg sign
      Serial.print((int)whole_voltage); //Index #7
      Serial.print(".");
      Serial.print("|");
    ...
    Serial.print(" %");
    Serial.print("|");  //Index #9    //ex output: "34 %"

    Serial.print((int)myMin);         /////////////Usage Time/////////////
    Serial.print(" m ");
    Serial.print("|"); //"0 m " index #10
    Serial.print((int)mySec);
    Serial.print(" s");
    Serial.print("|"); //"0 s" index #11   //ex output: "2 min 34 s %"
